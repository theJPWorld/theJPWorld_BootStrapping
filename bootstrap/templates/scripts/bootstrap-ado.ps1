...
$planResult = ./bootstrap/templates/scripts/tf-plan-local.ps1 -Workload adoFedSC -Environment "$Environment" -PAT $PAT
...
$applyResult = ./bootstrap/templates/scripts/tf-apply-local.ps1 -Workload adoFedSC -Environment "$Environment" -PAT $PAT
if ($LASTEXITCODE -ne 0) {
