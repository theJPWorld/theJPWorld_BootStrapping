...
./bootstrap/templates/scripts/tf-init-local.ps1 -Workload adoFedSC -AdoPAT $AdoPAT

# Run tf-plan for your User Managed Identities for Deployment
$planResult = ./bootstrap/templates/scripts/tf-plan-local.ps1 -Workload adoFedSC -Environment "$Environment" -AdoPAT $AdoPAT
...
$applyResult = ./bootstrap/templates/scripts/tf-apply-local.ps1 -Workload adoFedSC -Environment "$Environment" -AdoPAT $AdoPAT
if ($LASTEXITCODE -ne 0) {
